...
DEFINE_DEVICE_TYPE(NEOGEO_PVC_MSLUG5HB_CART, neogeo_pvc_mslug5hb_cart_device, "neocart_mslug5hb", "Neo Geo Metal Slug 5hb PVC Cart")

neogeo_pvc_mslug5hb_cart_device::neogeo_pvc_mslug5hb_cart_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	neogeo_pvc_cart_device(mconfig, NEOGEO_PVC_MSLUG5HB_CART, tag, owner, clock)
{
}

void neogeo_pvc_mslug5hb_cart_device::decrypt_all(DECRYPT_ALL_PARAMS)
{
	...
	if (memregion("cslot1:audiocrypt"))
		m_cmc_prot->cmc50_m1_decrypt(audiocrypt_region, audiocrypt_region_size, audiocpu_region, audio_region_size);

	ram = memregion("cslot1:ymsnd")->base();
	if (ram[0x60] != 0x82)
	{
		m_pcm2_prot->swap(ym_region, ym_region_size, 2);
	}

	ram = memregion("cslot1:sprites")->base();
	if (ram[0] != 0)
	{
		m_cmc_prot->cmc50_gfx_decrypt(spr_region, spr_region_size, MSLUG5_GFX_KEY);
	}

	ram = memregion("cslot1:fixed")->base();
	if (ram[0x100] == 0)
	{
		m_cmc_prot->sfix_decrypt(spr_region, spr_region_size, fix_region, fix_region_size);
	}
