...
/*************************************************
 EKMAME : this is plain NeoGeo cart + RAM
 **************************************************/

DEFINE_DEVICE_TYPE(NEOGEO_EK_CART, neogeo_ek_cart_device, "neocart_ek", "EK Neo Geo Cart")

neogeo_ek_cart_device::neogeo_ek_cart_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	neogeo_rom_device(mconfig, NEOGEO_EK_CART, tag, owner, clock),
	m_nvram(*this, "nvram")
{
}

void neogeo_ek_cart_device::device_start()
{
	m_cart_ram = make_unique_clear<uint16_t[]>(0x2000/2);
	m_nvram->set_base(m_cart_ram.get(), 0x2000);
	save_pointer(NAME(m_cart_ram), 0x2000/2);
...
void neogeo_ek_cart_device::device_add_mconfig(machine_config &config)
{
	NVRAM(config, m_nvram);
}
//#endif
