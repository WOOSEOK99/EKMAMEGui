...
DEFINE_DEVICE_TYPE(NEOGEO_MSLUG6S16_CART, neogeo_ms6s16_cart_device, "neocart_ms6s16", "Neo Geo Metal Slug 6 sx decrypt Cart")

neogeo_ms6s16_cart_device::neogeo_ms6s16_cart_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	neogeo_bootleg_cart_device(mconfig, NEOGEO_MSLUG6S16_CART, tag, owner, clock),
	m_cmc_prot(*this, "cmc_prot")
{
}

void neogeo_ms6s16_cart_device::decrypt_all(DECRYPT_ALL_PARAMS)
{
	m_prot->sx_decrypt(fix_region, fix_region_size, 2);
}

void neogeo_ms6s16_cart_device::device_add_mconfig(machine_config &config)
{
	NG_CMC_PROT(config, m_cmc_prot);
	NEOBOOT_PROT(config, m_prot);
}
