...
// const char* s_CSP1Gamefile[] =
// {
// 	"cps1.cpp",
// 	"cps1_hack.cpp"
// };

// const char* s_CSP2Gamefile[] =
// {
// 	"cps2.cpp"
// 	"cps2_hack.cpp"	
// };
...
// bool NEOGEOcombokeyCheck(device_t &device)
// {
// 	char *ptr=NULL;

// 	for(int i=0; i<25;i++)
// 	{
// 		ptr = strstr(device.source(),s_NeoGamefile[i]);	
// 		if(ptr)
// 		{
// 			return true;
// 		}
...
// bool CPS1combokeyCheck(device_t &device)
// {
// 	char *ptr=NULL;

// 	for(int i=0; i<2;i++)
// 	{
// 		ptr = strstr(device.source(),s_CSP1Gamefile[i]);	
// 		if(ptr)
// 		{
// 			return true;
// 		}
...
// bool CPS2combokeyCheck(device_t &device)
// {
// 	char *ptr=NULL;

// 	for(int i=0; i<2;i++)
// 	{
// 		ptr = strstr(device.source(),s_CSP2Gamefile[i]);	
// 		if(ptr)
// 		{
// 			return true;
// 		}
...
// bool PGMcombokeyCheck(device_t &device)
// {
// 	char *ptr=NULL;

// 	for(int i=0; i<2;i++)
// 	{
// 		ptr = strstr(device.source(),s_PGMGamefile[i]);	
// 		if(ptr)
// 		{
// 			return true;
// 		}
...
//	ioport_field *field;
	...
	for (auto &port : *this)
	{
		...
		{
			if (nplayer < field.player()+1) 	nplayer = field.player()+1;
		}
		port.second->collapse_fields(errorbuf);
	}
...
void ioport_port::insert_field(ioport_field &newfield, ioport_value &disallowedbits, std::string &errorbuf)
{
	...
	// 		errorbuf.append(string_format("INPUT_TOKEN_FIELD specifies duplicate port bits (port=%s Newmask=%X mask=%X)\n", tag(), newfield.mask(),disallowedbits));
	...
	for (ioport_field *field = m_fieldlist.first(); field != nullptr; field = nextfield)
	{
		...
		if ((field->mask() & newfield.mask()) != 0)
		{
			...
			//errorbuf.append(string_format("port=%s Newfield=%X, field=%X )\n", tag(), newfield.mask() , field->mask()  ) ); 		

			if(newfield.condition().none() || field->condition().none() || field->condition() == newfield.condition())
			{
