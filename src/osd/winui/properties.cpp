...
} g_PropSheets[] = 
{
	...
	{ IDD_PROP_OPENGL,		GameOptionsDialogProc },		// ë¹„ë””???¤ì •
...
void InitPropertyPage(HINSTANCE hInst, HWND hWnd, OPTIONS_TYPE opt_type, int folder_id, int game_num)
{
	...
	switch(opt_type)
	{
		case OPTIONS_GAME:
//#ifdef USE_KLIST
			snprintf(tmp, WINUI_ARRAY_LENGTH(tmp), "%s 속성", GetDescriptionByIndex(g_nGame,GetUsekoreanList()));
//else			
...
//#endif
		...
		case OPTIONS_SOURCE:
			snprintf(tmp, WINUI_ARRAY_LENGTH(tmp), "%s 드라이버 게임 속성", GetDriverFileName(g_nGame));
			break;
...
static char *GameInfoStatus(int driver_index)
{
	...
	{
		strcpy(buffer, "게임 동작 안합니다.");
		strcat(buffer, "\r\n");
		strcat(buffer, "게임이 제대로 작동하지 않습니다.");

		if (BIT(cache, 22))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "게임 Protection이 완전히 에뮬레이션되지 않았습니다.");
		}

		if (BIT(cache, 14))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "게임에 기계 부품이 있습니다.");
		}
	...
	{
		strcpy(buffer, "동작에 문제가 있습니다.");
		status_color = 2;

		if (BIT(cache, 21))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "색상이 완전히 다릅니다.");
		}

		if (BIT(cache, 20))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "색상이 100 % 정확하지 않습니다.");
		}

		if (BIT(cache, 18))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "비디오 에뮬레이션이 100 % 정확하지 않습니다.");
		}

		if (BIT(cache, 17))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "게임 사운드가 부족합니다.");
		}

		if (BIT(cache, 16))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "사운드 에뮬레이션이 100 % 정확하지 않습니다.");
		}

		if (BIT(cache, 15))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "게임을 완료할수 없습니다.");
		}

		if (BIT(cache, 13))
		{
			strcat(buffer, "\r\n");
			strcat(buffer, "사운드 하드웨어가 없습니다.");
		}
...
static char *GameInfoTitle(OPTIONS_TYPE opt_type, int nIndex)
{
	...
	if (OPTIONS_GLOBAL == opt_type)
		strcpy(buffer, "기본 게임 옵션 \r\n모든 게임에 기본설정으로 사용됩니다.");
	else if (OPTIONS_RASTER == opt_type)
		strcpy(buffer, "래스터 그래픽 게임 옵션 \r\n모든 래스터 그래픽 게임에 기본설정으로 사용됩니다.");
	else if (OPTIONS_VECTOR == opt_type)
		strcpy(buffer, "벡터 그래픽 게임 옵션\r\n모든 벡터 그래픽 게임에 기본설정으로 사용됩니다.");
	else if (OPTIONS_VERTICAL == opt_type)
		strcpy(buffer, "수직 방향 게임\r\n모든 수직 방향 게임에 기본설정으로 사용됩니다.");
	else if (OPTIONS_HORIZONTAL == opt_type)
		strcpy(buffer, "수평 방향 게임\r\n모든 수평 방향 게임에 기본설정으로 사용됩니다.");
	else if (OPTIONS_SOURCE == opt_type)
		strcpy(buffer, "드라이버 옵션\r\n드라이버 내의 모든 게임에 기본설정으로 사용됩니다.");
	else
//#ifdef USE_KLIST
...
//#endif
	...
	HFONT hTreeSheetFont = CreateFont(-11, 0, 0, 0, 400, 0, 0, 0, 0, 3, 2, 1, 18, TEXT("굴림"));
...
//#ifdef USE_KLIST
			snprintf(tmp, WINUI_ARRAY_LENGTH(tmp), "\"%s\" 정보", (char *)GetGameNameByIndex(index,GetUsekoreanList()));
//#else
