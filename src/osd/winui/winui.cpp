...
static wchar_t g_szPlayGameString[] = TEXT("실행 %s\tAlt+O");
static char g_szGameCountString[] = "%d 게임";
...
//	{6, ID_VIEW_GROUPED,  		TBSTATE_ENABLED, BTNS_CHECKGROUP, {0, 0}, 0, 6},
	{0, 0,                  	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0},
	{4, ID_ENABLE_INDENT,  		TBSTATE_ENABLED, BTNS_CHECK,      {0, 0}, 0, 6},
	{0, 0,                  	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0},
	{6, ID_UPDATE_GAMELIST,  	TBSTATE_ENABLED, BTNS_BUTTON,     {0, 0}, 0, 7},
	{0, 0,                    	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0},
	{7, ID_OPTIONS_INTERFACE,	TBSTATE_ENABLED, BTNS_BUTTON,     {0, 0}, 0, 8},
	{8, ID_OPTIONS_DEFAULTS, 	TBSTATE_ENABLED, BTNS_BUTTON,     {0, 0}, 0, 9},
	{0, 0,                   	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0},
	{9, ID_VIDEO_SNAP,			TBSTATE_ENABLED, BTNS_BUTTON,     {0, 0}, 0, 10},
	{10,ID_PLAY_M1,   			TBSTATE_ENABLED, BTNS_BUTTON,     {0, 0}, 0, 11},
	{0, 0,                    	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0},
	{14,ID_KOREAN_GAMELIST,    	TBSTATE_ENABLED, BTNS_CHECK,	  {0, 0}, 0, 12}, // USE_KLIST
	{0, 0,                    	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0}
};
...
static void Win32UI_init(void)
{
	...
	if (RequiredDriverCache())
		winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "Ini 파일 작성중...");
	else
		winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "Ini 파일 로딩중...");
	...
	winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "목록 작성중 ...");
	/* Initialize listview columns */
	InitListView();
	SendMessage(hProgress, PBM_SETPOS, 120, 0);
	winui_set_window_text_utf8(GetDlgItem(hSplash, IDC_PROGBAR), "초기화 중..");
	ResetFonts();
	AdjustMetrics();
	UpdateScreenShot();
...
static bool GameCheck(void)
{
	...
	{
		SetStatusBarTextW(0, TEXT("게임 찾는중 : %d%% 완료"), percentage);
...
static char *GameInfoStatusBar(int driver_index)
{
	...
	if (DriverIsBroken(driver_index))
		return strcpy(status, "동작 안함");
	else if (DriverIsImperfect(driver_index))
		return strcpy(status, "문제있음");
	else
		return strcpy(status, "실행 가능");
}
...
static bool MameCommand(HWND hWnd, int id, HWND hWndCtl, UINT codeNotify)
{
	...
	case ID_NOTEPAD:
		...
		case ID_HELP_TROUBLE:
			ShellExecuteCommon(hMain, "https://cafe.naver.com/emulk");
			SetFocus(hWndList);
			return true;
