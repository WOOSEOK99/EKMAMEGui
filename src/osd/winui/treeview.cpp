...
{
    //e??i?????char* eÂ¹?A????iï¿?eÂ¼Î¼
    char* pStr ; 

    //???¦i?Â°e??i??? wchar_t eÂ¹?A????i?œÂ?Ã¦Â¹Â²eÂª???c?˜Å??Â´N?eÂ¸Â¿
    int strSize = WideCharToMultiByte(CP_ACP, 0,str,-1, NULL, 0,NULL, NULL);

    //char* iÂ§?eÂ¶Â¾??c?Â??iï¿?e?|
...
extern const FOLDERDATA g_folderData[] =
{
	...
	{"모든 게임",       "allgames",          FOLDER_ALLGAMES,     IDI_FOLDER_ALLGAMES,      0,             0,            0, NULL,                       NULL,                    true },
	{"보유 게임",       "available",         FOLDER_AVAILABLE,    IDI_FOLDER_AVAILABLE,     F_AVAILABLE,   0,            0, NULL,                       FilterAvailable,         true },
	{"미보유 게임",     "unavailable",       FOLDER_UNAVAILABLE,  IDI_FOLDER_UNAVAILABLE,   0,             F_AVAILABLE,  0, NULL,                       FilterAvailable,         false },
//#ifdef USE_GAMEFOLDERS
	{"한국어 게임",	  "korean",		 		FOLDER_KOREAN,		IDI_FOLDER_KOREAN,		  0,			 0, 		   0, CreateKOREANFolders },
...
//#endif 
	{"원작 게임",       "originals",       FOLDER_ORIGINAL,     IDI_FOLDER_ORIGINALS,     F_ORIGINALS,   F_CLONES,     0, NULL,                       DriverIsClone,           false },
	{"복제 게임",       "clones",          FOLDER_CLONES,       IDI_FOLDER_CLONES,        F_CLONES,      F_ORIGINALS,  0, NULL,                       DriverIsClone,           true },
	{"소스",          	"source",      FOLDER_SOURCE,       IDI_FOLDER_SOURCE,        0,             0,            0, CreateSourceFolders },

	{"세로방향 게임",   "vertical",          FOLDER_VERTICAL,     IDI_FOLDER_VERTICAL,      F_VERTICAL,    F_HORIZONTAL, 0, NULL,                       DriverIsVertical,        true },
	{"가로방향 게임",   "horizontal",        FOLDER_HORIZONTAL,   IDI_FOLDER_HORIZONTAL,    F_HORIZONTAL,  F_VERTICAL,   0, NULL,                       DriverIsVertical,        false },
	{"실행 가능",       "working",         FOLDER_WORKING,      IDI_WORKING,              F_WORKING,     F_NONWORKING, 0, NULL,                       DriverIsBroken,          false },
	{"실행 불가능",     "nonworking",       FOLDER_NONWORKING,   IDI_NONWORKING,           F_NONWORKING,  F_WORKING,    0, NULL,                       DriverIsBroken,          true },
	{"불완전 게임",     "imperfect",        FOLDER_DEFICIENCY,   IDI_FOLDER_IMPERFECT,     0,             0,            0, CreateDeficiencyFolders },
	{"년도별 게임",     "year",             FOLDER_YEAR,         IDI_FOLDER_YEAR,          0,             0,            0, CreateYearFolders },
	{"제조사별 게임",   "manufacturer",      FOLDER_MANUFACTURER, IDI_FOLDER_MANUFACTURER,  0,             0,            0, CreateManufacturerFolders },
	{"바이오스별 게임", "bios",               FOLDER_BIOS,         IDI_FOLDER_BIOS,          0,             0,            1, CreateBIOSFolders,          DriverIsBios,            true },
	{"하드디스크 게임", "harddisk",           FOLDER_HARDDISK,     IDI_HARDDISK,             0,             0,            0, NULL,                       DriverIsHarddisk,        true },
	{"CPU별 게임",      "cpu",            FOLDER_CPU,          IDI_FOLDER_CPU,           0,             0,            1, CreateCPUFolders },
	{"라이트건 게임",  "lightgun",          FOLDER_LIGHTGUN,     IDI_FOLDER_LIGHTGUN,      0,             0,            0, NULL,                       DriverUsesLightGun,      true },
	{"트랙볼 게임",     "trackball",        FOLDER_TRACKBALL,    IDI_FOLDER_TRACKBALL,     0,             0,            0, NULL,                       DriverUsesTrackball,     true },
	{"래스터 그래픽",   "raster",            FOLDER_RASTER,       IDI_FOLDER_RASTER,        F_RASTER,      F_VECTOR,     0, NULL,                       DriverIsVector,          false },
	{"벡터 그래픽",     "vector",           FOLDER_VECTOR,       IDI_FOLDER_VECTOR,        F_VECTOR,      F_RASTER,     0, NULL,                       DriverIsVector,          true },
	{"게임 저장 지원",  "savestate",         FOLDER_SAVESTATE,    IDI_FOLDER_SAVESTATE,     0,             0,            0, CreateSaveStateFolders },
	{"덤프 상태",  			"dumping",     FOLDER_DUMPING,      IDI_FOLDER_DUMP,          0,             0,            1, CreateDumpingFolders },
	...
	{"재생률(fps)",     "refresh",        FOLDER_FPS,          IDI_FOLDER_FPS,           0,             0,            1, CreateFPSFolders },
	{"해상도",      		"resolution",  FOLDER_RESOLUTION,   IDI_FOLDER_RESOL,         0,             0,            1, CreateResolutionFolders },
	{"사운드",          "sound",          FOLDER_SOUND,        IDI_FOLDER_SOUND,         0,             0,            1, CreateSoundFolders },
	{"샘플",         		"samples",     FOLDER_SAMPLES,      IDI_FOLDER_SAMPLES,       0,             0,            0, NULL,                       DriverUsesSamples,       true },
	{"화면",         		"screens",     FOLDER_SCREENS,      IDI_FOLDER_MONITOR,       0,             0,            0, CreateScreenFolders },
...
static void CreateDeficiencyFolders(int parent_index)
{
	...
	lpWrongCol = NewFolder("색상이 잘못됨", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpImpCol = NewFolder("불완전한 색상", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpImpGraph = NewFolder("불완전한 그래픽", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpMissSnd = NewFolder("사운드 부족", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpImpSnd = NewFolder("불완전한 사운드", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpIncomplete = NewFolder("불완전한 프로토타입", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpNoSndHw = NewFolder("사운드 하드웨어 없음", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
...
static void CreateSaveStateFolders(int parent_index)
{
	...
	lpSupported = NewFolder("지원함", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpUnsupported = NewFolder("지원안함", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
...
static void CreateResolutionFoldersIni(int parent_index)
{
	...
	SetAllBits(lpFolder->m_lpGameBits, false);
	// create our two subfolders
	lpVectorH = NewFolder("벡터 (H)", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpVectorV = NewFolder("벡터 (V)", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
...
static void CreateFPSFoldersIni(int parent_index)
{
	...
	SetAllBits(lpFolder->m_lpGameBits, false);
	// create our two subfolders
	lpVector = NewFolder("벡터", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpScreenless = NewFolder("Screenless", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	AddFolder(lpVector);
	AddFolder(lpScreenless);
...
void CreateDumpingFoldersIni(int parent_index)
{
	...
	SetAllBits(lpFolder->m_lpGameBits,false);
	// create our two subfolders
	lpNoDump = NewFolder("덤프 안됨", next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpBadDump = NewFolder("덤프 잘못됨", next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	lpGoodDump = NewFolder("덤프 잘됨", next_folder_id, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	AddFolder(lpNoDump);
	AddFolder(lpBadDump);
...
void CreateTAITOFolders(int parent_index)
{
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("rainbow.cpp", s))			AddGame(lpFolder, jj);
		...
		if (!strcmp("mexico86.cpp", s))			AddGame(lpFolder, jj);
...
void CreateKONAMIFolders(int parent_index)
{
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("gbusters.cpp", s))			AddGame(lpFolder, jj);
...
void CreateSEGAFolders(int parent_index)
{
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("system24.cpp", s))			AddGame(lpFolder, jj);
...
void CreateTOAFolders(int parent_index)
{
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("hack_cave.cpp", s))				AddGame(lpFolder, jj);
		if (!strcmp("kr_cave.cpp", s))				AddGame(lpFolder, jj);
