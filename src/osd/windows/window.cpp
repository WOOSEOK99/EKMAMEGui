...
#include "modules/render/drawdd.h"
...
//DDRAW
//static direct_draw_callback draw;
//extern int drawdd_init(running_machine &machine, direct_draw_callback *callbacks);
...
bool windows_osd_interface::window_init()
{
	...
	const int fallbacks[VIDEO_MODE_COUNT] = {
		...
		VIDEO_MODE_DDRAW,     // BGFX -> D3D		//DDRAW
#if (USE_OPENGL)
		...
		switch(current_mode)
	{
			...
			case VIDEO_MODE_GDI:
				...
				break;
			case VIDEO_MODE_DDRAW:
				error = renderer_dd::init(machine());	//DDRAW
				break;						
...
std::shared_ptr<osd_monitor_info> win_window_info::monitor_from_rect(const osd_rect* proposed)
{
	std::shared_ptr<osd_monitor_info> monitor;
...
void avi_info_view(running_machine &machine)
{
	...
	const screen_device *screen = screen_device_iterator(config.root_device()).first();//(const screen_device *)downcast<const legacy_device_config_base &>(machine->primary_screen->baseconfig()).inline_config();
...
int avi_write_handler(running_machine *machine, emu_file *dummy, bitmap_t *bitmap)
{
    if (bAviRun)
	{
		...
		if ((!machine->paused()) || (machine->paused() && single_step()))
		{
