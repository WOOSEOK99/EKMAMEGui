...
//EKMAME
void menu::item_append_ex(const char *text, const char *subtext, uint32_t flags, void *ref)
{
	// only allow multiline as the first item
	if ((flags & FLAG_MULTILINE) != 0)
		assert(m_items.size() == 1);
			...
			else
			{
				...
				else
				{
					...
					if (!core_stricmp(subitem_text, _("On")))
						fgcolor2 = UI_GREEN_COLOR;  // MAMEFX

					if (!core_stricmp(subitem_text, _("Off")))
						fgcolor2 = UI_RED_COLOR;   // MAMEFX

					if (!core_stricmp(subitem_text, _("Auto")))
						fgcolor2 = UI_YELLOW_COLOR;  // MAMEFX
